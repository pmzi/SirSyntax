module.exports = function (bot) {
    ...
    bot.findQuestionsByTeacherID = (teacherID) => {
        return new Promise((resolve, reject) => {
            Teachers.findById(teacherID, "gCat").then((ts) => {
    ...
    bot.getLessensFromIDS = (lessenIDS) => {
        return new Promise((resolve, reject) => {

            if (lessenIDS.length == 0) {
                resolve([]);
            }

            let lessens = [];

            lessenIDS.forEach((lessenID) => {
                Lessens.findById(lessenID).then((lessen) => {
                    lessens.push(lessen);

                    //if the end

                    if (lessens.length == lessenIDS.length) {
                        resolve(lessens); //What the fuck?
                    }
    ...
    bot.addCallBack("cb_lessen", /^lessen-([\d\w]+)-([\d\w]+)$/i, (data, msg) => {
        //showing the lessens that the teacher have thought
        let teacherID = data[1];
        ...
        Votes.find({
            ...
            Likes.find({
                ...
                bot.findQuestionsByTeacherID(teacherID).then((questions) => {
                        Teachers.findTeacherName(teacherID).then((teacherName) => {

                            let result = voteParser.parse(vs, questions, lks, teacherName);

                            const keyboard = bot.createInlineKeyboard([{
                                name: "پیشنهاد می‌دهم",
                                _id: 0 + "-" + lessenID + "-" + teacherID
                            }, {
                                name: "پیشنهاد نمی‌دهم",
                                _id: 1 + "-" + lessenID + "-" + teacherID
                            }, {
                                name: "مشاهده نظرات",
                                _id: 2 + "-" + lessenID + "-" + teacherID
                            }, {
                                name: "ثبت نظر",
                                _id: 3 + "-" + lessenID + "-" + teacherID
                            }], "vote-see");
    ...
    bot.addCallBack("cb_vote_see", /^vote-see-([\d\w]+)-([\w\d]+)-([\w\d]+)$/i, (data, msg) => {
